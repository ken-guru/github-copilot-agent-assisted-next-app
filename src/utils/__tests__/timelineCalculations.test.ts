...
describe('calculateTimeSpans', () => {
  ...
  it('should return a single activity item with the correct duration and height', () => {
    ...
    expect(result.items[0].height).toBeCloseTo(((endTime - startTime) / (totalDuration * 1000)) * 100);
  });

  it('should return multiple activity items with the correct durations and heights', () => {
    const now = Date.now();
    const startTime1 = now - 20000;
    const endTime1 = now - 10000;
    const startTime2 = now - 10000;
    const endTime2 = now;
    ...
    expect(result.items[0].height).toBeCloseTo(((endTime1 - startTime1) / (totalDuration * 1000)) * 100);

    expect(result.items[1].type).toBe('activity');
    expect(result.items[1].duration).toBe(endTime2 - startTime2);
    expect(result.items[1].height).toBeCloseTo(((endTime2 - startTime2) / (totalDuration * 1000)) * 100);
  });

  it('should return multiple activity items with gaps', () => {
    const now = Date.now();
    const startTime1 = now - 30000;
    const endTime1 = now - 20000;
    const startTime2 = now - 10000;
    const endTime2 = now;
    ...
    expect(result.items[1].height).toBeCloseTo(((startTime2 - endTime1) / (totalDuration * 1000)) * 100);
  });
  ...
  it('should include a gap item for the remaining time when all activities are completed', () => {
    ...
    expect(result.items.length).toBe(2);
    expect(result.items[1].type).toBe('gap');
    expect(result.items[1].duration).toBe(timeLeft * 1000);
    expect(result.items[1].height).toBeCloseTo(((timeLeft * 1000) / (totalDuration * 1000)) * 100);
  });

  it('should use the `now` parameter to calculate the duration for entries with `endTime` undefined', () => {
    const now = Date.now();
    const startTime = now - 10000;
    const entries: TimelineEntry[] = [
      {
    ...
    expect(result.items.length).toBe(1);
    expect(result.items[0].type).toBe('activity');
    expect(result.items[0].duration).toBe(now - startTime);
    expect(result.items[0].height).toBeCloseTo(((now - startTime) / (totalDuration * 1000)) * 100);
  });
});
